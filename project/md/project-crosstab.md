...
CROSSTAB - сокращение от cross-tabulation, т.е. перекрёстная таблица (суммарная таблица).
...
```
Здесь также используется кастомная агрегация массивов, как и в случае CASE WHEN преобразованием.
...
В контексте плана выполнения запроса Function Scan — это механизм, в котором PostgreSQL обрабатывает результаты, 
возвращаемые функцией crosstab.